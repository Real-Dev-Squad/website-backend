...
- Copy the response cookie named `rds-session` (`rds-session-development` in development mode) from the redirected request <HOST>/auth/github/callback?code=<SOME_AUTOGENERATED_CODE>
- Once authorized, simply go to the Application tab of your browser's developer tools and boom! There's your 🍪.
