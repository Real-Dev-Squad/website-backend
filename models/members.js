...
const addArchiveRoleToMembers = async (userId) => {
  try {
    const userDoc = await userModel.doc(userId).get()
    const user = userDoc.data()
    if (user?.roles && user?.roles?.archivedMember) return { isArchived: true, addArchiveRole: false }
    const roles = user.roles ? { ...user.roles, archivedMember: true } : { archivedMember: true }
    await userModel.doc(userId).update({
      roles
    })
    return { isArchived: false, addArchiveRole: true }
  } catch (err) {
    logger.error('Error updating user', err)
    throw err
  }
