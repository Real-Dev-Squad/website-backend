...
import { OnboardingExtension, UpdateOnboardingExtensionRequestBody } from "../types/onboardingExtension.js";
...
export const validateOnboardingExtensionUpdateRequest = async (
  ...
  try {
    ...
    if (!isSuperuser && lastModifiedBy !== extensionRequest.userId) {
      await addLog(
        logType.UNAUTHORIZED_TO_UPDATE_REQUEST,
    ...
    if (extensionRequest.type !== REQUEST_TYPE.ONBOARDING) {
      await addLog(logType.INVALID_REQUEST_TYPE, { type: extensionRequest.type }, { message: INVALID_REQUEST_TYPE });
      return {
    ...
    if (extensionRequest.state !== REQUEST_STATE.PENDING) {
      await addLog(
        logType.PENDING_REQUEST_CAN_BE_UPDATED,
    ...
    if (extensionRequest.oldEndsOn >= newEndsOn) {
      await addLog(
        logType.INVALID_REQUEST_DEADLINE,
