...
describe('health', function () {
  ...
  it('should return uptime from the healthcheck API', function (done: any) {
    chai
      .request(app)
      .get('/healthcheck')
      .end((err: any, res: any) => {
  ...
  // @ts-expect-error ts-migrate(2582) FIXME: Cannot find name 'it'. Do you need to install type... Remove this comment to see the full error message
  it('should return 401 from the authenticated healthcheck API for missing auth tokens', function (done: any) {
    chai
      .request(app)
      .get('/healthcheck/v2')
      .end((err: any, res: any) => {
