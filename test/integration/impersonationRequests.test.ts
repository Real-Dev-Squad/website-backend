...
import { CreateImpersonationRequestBody, CreateImpersonationRequestModelDto } from "../../types/impersonationRequest";
...
describe("Impersonation Requests", () => {
  ...
  beforeEach(async () => {
    const userIdPromises = [addUser(userData[16]), addUser(userData[18]), addUser(userData[12]), addUser(userData[4])];
    ...
    const { id: impersonationRequestId1 } = await requestQuery.createImpersonationRequest({
      ...impersonationRequestsBodyData[0],
      impersonatedUserId: testUserId2,
      userId: superUserId,
    });
    const { id: impersonationRequestId2 } = await requestQuery.createImpersonationRequest({
      ...impersonationRequestsBodyData[0],
