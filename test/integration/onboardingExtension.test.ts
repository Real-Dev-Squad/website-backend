...
describe("/requests Onboarding Extension", () => {
  describe("POST /requests", () => {
    ...
    it("should return 500 response when fails to create extension request", (done) => {
      ...
      sinon.stub(requestsQuery, "createRequest").throws("Error while creating extension request");
      chai
        .request(app)
        .post(postEndpoint)
        .set("authorization", `Bearer ${botToken}`)
    ...
    it("should return 201 response when previous latest extension request is approved", async () => {
      ...
      const res = await chai.request(app).post(postEndpoint).set("authorization", `Bearer ${botToken}`).send(body);
      ...
      expect(res.body.data.newEndsOn).to.equal(
        latestApprovedExtension.newEndsOn + body.numberOfDays * 24 * 60 * 60 * 1000
      );
    });
    ...
    it("should return 201 response when previous latest extension request is rejected", async () => {
      ...
      expect(new Date(res.body.data.newEndsOn).toDateString()).to.equal(
        new Date(currentDate + body.numberOfDays * 24 * 60 * 60 * 1000).toDateString()
      );
    });
  });
  ...
  describe("PUT /requests", () => {
    ...
    it("should return 500 response when fails to update extension request", (done) => {
      sinon.stub(requestsQuery, "updateRequest").throws("Error while creating extension request");
      chai
        .request(app)
        .put(putEndpoint)
        .set("authorization", `Bearer ${authToken}`)
