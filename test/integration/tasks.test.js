...
describe("Tasks", function () {
  ...
  describe("GET /tasks/self", function () {
    ...
    it("Should return all assignee task", async function () {
      ...
      const { taskId: taskId1 } = await tasks.updateTask(await toFirestoreData(assignedTask[0]));
      const { taskId: taskId2 } = await tasks.updateTask(await toFirestoreData(assignedTask[1]));
  ...
  describe("PATCH /tasks/self/:id", function () {
    ...
    it("Should give 403 if status is already 'VERIFIED' ", async function () {
      ...
      taskId = (await tasks.updateTask(await toFirestoreData(taskData))).taskId;
      const res = await chai
        .request(app)
  ...
  describe("GET /tasks/overdue", function () {
    it("Should return all the overdue Tasks", async function () {
      await tasks.updateTask(await toFirestoreData(tasksData[0]));
      await tasks.updateTask(await toFirestoreData(tasksData[1]));
    ...
    it("Should return [] if no overdue task", async function () {
      await tasks.updateTask(await toFirestoreData(tasksData[2]));
      const res = await chai.request(app).get("/tasks/overdue").set("cookie", `${cookieName}=${superUserJwt}`);
