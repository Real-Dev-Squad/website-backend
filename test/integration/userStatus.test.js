...
describe("UserStatus", function () {
  ...
  describe("PATCH /users/status/:userid", function () {
    ...
    it("Should store the User Status in the collection", async function () {
      ...
      const userStatus = await firestore.collection("usersStatus").where("userId", "==", testUserId).get();
      expect(userStatus.size).to.equal(1);
      const data = userStatus.docs[0].data();
      expect(data.currentStatus.state).to.equal("ONBOARDING");
    });
