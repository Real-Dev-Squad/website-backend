...
describe("tasks", function () {
  ...
  describe("overdueTasks", function () {
    it("Should return overdue tasks", async function () {
      const { taskDetails: taskData1, taskId: taskId1 } = await tasks.updateTask(await toFirestoreData(tasksData[0]));
      const { taskDetails: taskData2, taskId: taskId2 } = await tasks.updateTask(await toFirestoreData(tasksData[1]));
